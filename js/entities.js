...
class SuperBumper {
  ...
  draw(ctx) {
    ...
    ctx.save();
    ctx.shadowBlur = LOW_END_MODE ? 0 : ((this.pulse > 0.05 ? 8 + this.pulse * 10 : 4) * QUALITY);
    ctx.shadowColor = this.color;
    ctx.fillStyle = grad;
    ctx.beginPath();
...
class LaunchPad {
  ...
  draw(ctx) {
    ...
    ctx.shadowBlur = LOW_END_MODE ? 0 : ((this.pulse > 0.05 ? 6 + this.pulse * 12 : 4) * QUALITY);
    ctx.shadowColor = this.color;
...
class Peg {
  ...
  draw(ctx) {
    ...
    ctx.shadowBlur = LOW_END_MODE ? 0 : ((this.pulse * 12) * QUALITY);
...
class SpikeTrap {
  ...
  draw(ctx) {
    ...
    ctx.shadowBlur = LOW_END_MODE ? 0 : ((8 + danger * 10) * QUALITY);
    ...
    ctx.shadowBlur = LOW_END_MODE ? 0 : (14 * QUALITY);
    ctx.beginPath();
...
class ItemBase {
  ...
  draw(ctx) {
    ...
    ctx.translate(this.x, this.y);
    ctx.rotate(this.angle);
    ctx.shadowBlur = LOW_END_MODE ? 0 : ((8 + glow * 9) * QUALITY);
